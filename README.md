...
> **實作採用的是下方「實際營運狀態」（業主 2026-09 確認），不是本節原本的名稱。**
> 唯一真相在資料庫：`order_status_rules` ＋ `trg_order_status_check`（見
> `supabase/migrations/001_init.sql`），程式繞不過去。
>
> | 實際營運（採用） | 允許轉換 | 原本對應 |
> |---|---|---|
> | 待確認 | → 已報價、已取消 | 待付款 |
> | 已報價 | → 已到貨、缺貨、已取消 | 待採購（含原「部分到貨」：買不齊就停在這裡） |
> | 已到貨 | → 已出貨 | 待出貨 |
> | 已出貨 | → 已送達 | 已出貨 |
> | 已送達／缺貨／已取消 | 終點 | 已完成／待退款＋已退款／已取消 |
>
> 通知轉換改為 `已到貨 → 已出貨`；店主倒退修正改以交易內
> `app.override='on'` ＋ `app.actor`／`app.reason` 放行並留紀錄。
> 以下原圖保留作為需求來源對照。
