 ...
 */
async function runChecks(raw) {
  const parsed = sig.parse(raw);
  if (!parsed) return { level: 'red', code: 'BAD_FORMAT', step: 1, message: '格式不是本店標籤' };

  const order = await db.one('SELECT * FROM orders WHERE order_id = ?', parsed.orderId);
  ...
  const context = {
    ...
    status: order.status,
    nickname: ((await db.one('SELECT nickname FROM members WHERE line_user_id = ?', order.line_user_id)) || {}).nickname,
    items: (await itemsOf(order.order_id)).map((i) => ({ sku: i.sku, name_zh: i.name_zh, qty: i.qty })),
  };
  ...
  const coverage = await procurementCoverage(order.order_id);
...
post('/api/v1/scan/verify', async ({ actor, body }) => {
  auth.requireCap(actor, 'shipment.write');
  const result = await runChecks(body.code);
  await audit.record({ actor: actor.line_user_id, action: 'scan.verify', target: result.order_id || null,
 ...
 */
post('/api/v1/scan/commit', async ({ actor, body }) => {
  auth.requireCap(actor, 'shipment.write');
  const result = await runChecks(body.code);
  const reason = body.override_reason ? String(body.override_reason).trim() : null;

  if (result.level === 'red') {
    await audit.record({ actor: actor.line_user_id, action: 'scan.commit', target: result.order_id || null, detail: { check: result.code }, result: 'blocked' });
    throw err(result.code, `${result.message}（紅燈不可覆寫）`, 409);
  }
  if (result.level === 'yellow') {
    if (!auth.can(actor, 'override.blocked')) {
      await audit.record({ actor: actor.line_user_id, action: 'scan.commit', target: result.order_id, detail: { check: result.code }, result: 'blocked' });
  ...
  const order = await db.one('SELECT * FROM orders WHERE order_id = ?', result.order_id);
...
get('/api/v1/labels/data', async ({ actor, query }) => {
  ...
  if (ids.length) {
    rows = [];
    for (const id of ids) {
      const row = await db.one(`SELECT o.*, m.nickname FROM orders o JOIN members m ON m.line_user_id = o.line_user_id WHERE o.order_id = ?`, id);
      if (row) rows.push(row);
    }
  } else {
    rows = await db.all(`SELECT o.*, m.nickname FROM orders o JOIN members m ON m.line_user_id = o.line_user_id WHERE o.status = ? ORDER BY o.created_at`, STATUS.ARRIVED);
  }
  ...
  for (const o of rows) {
    ...
    const items = await itemsOf(o.order_id);
    labels.push({
      order_id: o.order_id,
      nickname: o.nickname,
  ...
  await audit.record({ actor: actor.line_user_id, action: 'label.print', detail: { count: labels.length }, result: 'ok' });
  return ok({ labels });
});
